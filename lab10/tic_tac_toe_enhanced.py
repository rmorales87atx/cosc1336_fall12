...
class Game:
    ...
    def move_computer(self):
        ...
        if board.symbol(rx, cx) == O and X in board.corners() and O not in board.corners():
...
class Application:
    def __init__(self, parent):
        ...
        self.buttons = [[None for cx in range(self.bsize)] for rx in range(self.bsize)]
        ...
        if game.move_user(row_index, col_index):
            ...
            if over:
                if winner is None:
                    ...
                    self.set_status("Game over: you won!")
                elif winner == game.sym_cpu:
                    self.set_status("Game over: you lost.")
        else:
